<template>
  <div class="bankcard"><!-- 这是根节点 -->
	<div class="bankcard-inner">
		<h3 class="bankcard-title">更换银行卡</h3>
...
            },
	...
	methods:{
		...
		addCard(){
			...
			var oldCardDetail = sessionStorage.get("oldCardDetails");
			console.log("ccccc : " + JSON.stringify(oldCardDetail));

			var channelid_old = oldCardDetail.channelid;
			var depositacct_old = oldCardDetail.depositacct;
			var moneyaccount = oldCardDetail.moneyaccount;

			this.$httpPost("/pc/myaccount/identifyBackSendChangeFy",{accountName:this.custName, mobileno:this.form.bankPhone, tpasswd:this.form.password,
				certificateno:this.form.card, channelid:channelid, depositacct:this.form.bankId, pro_text:pro_text, pro_value:pro_value,
				city_text:city_text, city_value:city_value, bankName:bankName, validCode:this.form.checkCode, moneyaccount:moneyaccount,
				channelid_old:channelid_old,depositacct_old:depositacct_old
				}).then(res => {
